...
class ModelRequest:
    ...
    evidence_trust: str = "UNTRUSTED_CANONICAL_DATA"
    ...
    def __post_init__(self) -> None:
        ...
        if self.instruction_profile != "COPILOT_GROUNDED_V0_1":
            raise ModelPolicyError("unsupported instruction profile")
        if self.evidence_trust != "UNTRUSTED_CANONICAL_DATA":
            raise ModelPolicyError("retrieved evidence must remain data, never runtime instruction")
        if self.external_tools_allowed:
            raise ModelPolicyError("external model tools are forbidden in Copilot alpha")
        chunk_ids = tuple(item.chunk_id for item in self.evidence)
        if len(chunk_ids) != len(set(chunk_ids)):
            raise ModelPolicyError("duplicate evidence chunk ids are ambiguous")
...
class GuardedModelAdapter:
    ...
    def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        if response.tool_calls:
            raise ModelPolicyError("model tool calls are forbidden")
        if response.external_source_ids:
            raise ModelPolicyError("external sources are forbidden")
    ...
    def _request_hash(request: ModelRequest) -> str:
        payload = "|".join(
            [
                ...
                request.evidence_trust,
                str(request.external_tools_allowed),
